...
const PostItem: React.FC<PostItemProps> = ({data, userId}) => {
    ...
    const goToUser = React.useCallback((event: any) => {
        event.stopPropagation()

        router.push(`/users/${data.users?.id}`)
    }, [router, data.users?.id])

    const goToPost = React.useCallback(() => {
        router.push(`/posts/${data.id}`)
    }, [router, data.id])

    const onLike = React.useCallback((event: any) => {
        event.stopPropagation()
                ...
                <div>
                    <div className={'flex flex-row items-center gap-2'}>
                        ...
                        <span onClick={goToUser}
                              className={'text-neutral-500 cursor-pointer hover:underline md:block hidden'}>@{data.user.name}</span>
